            ...
            <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg px-4 py-4">
                ...
                <div class="bg-white">
                    <div class="mx-auto max-w-2xl py-12 px-4 sm:py-24 sm:px-6 lg:max-w-7xl lg:px-8">
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-4 gap-x-12">

                            <div class="grid col-span-3">
                                {{-- <h1>Productos</h1> --}}

                                <div
                                    class="grid grid-cols-1 gap-y-10 gap-x-6 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 h-[330px]">
                                    {{-- xl:grid-cols-4 xl:gap-x-8 --}}
                                    @foreach ($productos as $producto)
                                        <a href="{{ route('productos.show', [$categoria->slug, $producto]) }}"
                                            class="h-full">
                                            <x-producto image="{{ $producto->imagen }}" nombre="{{ $producto->nombre }}"
                                                precioLista="{{ number_format($producto->precioLista, 2) }}"
                                                precioOferta="{{ number_format($producto->precioOferta, 2) }}"
                                                fechahoy="{{ $fechahoy }}"
                                                ofertaDesde="{{ $producto->ofertaDesde }}"
                                                ofertaHasta="{{ $producto->ofertaHasta }}" />
                                        </a>
                                    @endforeach

                                    {{-- <a href="{{ route('productos.show', [$categoria->slug, $producto]) }}" class="group">
										<div
											class="aspect-w-1 aspect-h-1 w-full overflow-hidden rounded-lg bg-gray-200 xl:aspect-w-7 xl:aspect-h-8">

											<picture>
												<source srcset="{{ asset('storage/productos/' . $producto->imagen) }}" type="image/webp">
                            ...
                            <div class="grid grid-cols-1">
                                ...
                                <div class="px-6 lg:px-0">
                                    ...
                                    <div class="mt-5">
                                        ...
                                        {{-- <div class="flex items-center mt-3">
											<span>
												<svg xmlns="http://www.w3.org/2000/svg" fill="rgb(185,28,28)" height="24" viewBox="0 96 960 960"
													width="32">
